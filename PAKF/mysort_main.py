...
from my_sort.mysort import Mysort
...
def parse_opt():
    ...
    # 存放需要检测数据的文件夹
    # parser.add_argument('--data_path', default='./yolox/dataset/MOT17-13', help='data path')  #02/04/05/09/10/11/13
    # 存放跟权重相应模型的配置文件
    parser.add_argument('--exp_file', default='./yolox/exp/yolox_x_mix_det', help='exp path')  #样例文件
    # 存放模型权重的路径
    parser.add_argument('--weight_path', default='./yolox/weights/bytetrack_x_mot17.pth.tar', help='weight path') #权重的位置
    parser.add_argument("--save",default=False,help='save results')
    parser.add_argument('--save_txt', default=False, help='save txt results')
    # parser.add_argument('--save_dir', default='outdata/runs/sort/mot17_13', help='save results dir')
    parser.add_argument('--show', default=False, help='show frame img')
    # parser.add_argument('--eval_path',default='outdata/runs/sort/MOT17-13-FRCNN',help='eval_path')
...
def parse_args():
    ...
    parser.add_argument("--min_box",default=100,help="min_box_area")

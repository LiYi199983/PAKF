...
class PAkalmanFilter(object):
    ...
    def predict(self, mean, covariance, k):
        ...
        std_pos = [
            self._std_weight_position * mean[3]*k,
            self._std_weight_position * mean[3]*k,
            1e-2 * k,
            self._std_weight_position * mean[3]*k]
        std_vel = [
            self._std_weight_velocity * mean[3]*k,
            self._std_weight_velocity * mean[3]*k,
            1e-5*k,
            self._std_weight_velocity * mean[3]*k]
    ...
    def project(self, mean, covariance, scores):
        ...
        """
        k_r = 1 - scores ** self.beta
        std = [
            self._std_weight_position * mean[3],
            self._std_weight_position * mean[3],
            1e-1,
            self._std_weight_position * mean[3]]
        innovation_cov = k_r * np.diag(np.square(std))
    ...
    def update(self, mean, covariance, measurement, scores, cost_IOU_measurement):
        ...
        """
        projected_mean, projected_cov = self.project(mean, covariance, scores)
        ...
        IOU = 1-cost_IOU_measurement
        new_k = 1-IOU**self.alphe
        return new_mean, new_covariance, new_k
